...
def Create(game):
    print('Adding rooms')
    game.addRoom(Room(1,'Starboard Bows',{"DOWN":16,"SOUTH":2},[]))
    game.addRoom(Room(2,'Port Bows',{"NORTH":1},[]))
    game.addRoom(Room(3,'Starboard Forebeam',{"DOWN":18,"SOUTH":4},[]))
    game.addRoom(Room(4,'Starboard Foredeck',{"NORTH":3,"SOUTH":5},[]))
    game.addRoom(Room(5,'Port Foredeck',{"NORTH":4,"SOUTH":6},[]))
    game.addRoom(Room(6,'Port Forebeam',{"NORTH":5,"EAST":10},[]))
    game.addRoom(Room(7,'Starboard Beam',{"EAST":11,"SOUTH":8},[]))
    game.addRoom(Room(8,'Starboard Midships',{"NORTH":7},[]))
    game.addRoom(Room(9,'Port Midships',{},[]))
    game.addRoom(Room(10,'Port Beam',{"NORTH":9,"EAST":14,"WEST":6},[]))
    game.addRoom(Room(11,'Starboard Quarter',{"SOUTH":12,"WEST":7},[]))
    game.addRoom(Room(12,'Starboard Aft Deck',{"NORTH":11,"SOUTH":13,"DOWN":20},[]))
    game.addRoom(Room(13,'Port Aft Deck',{"NORTH":12,"SOUTH":14},[]))
    game.addRoom(Room(14,'Port Quarter',{"NORTH":13,"WEST":10},[]))
    game.addRoom(Room(15,'Poop Deck',{"DOWN":22,"EAST":23},[]))
    game.addRoom(Room(16,'Starboard Forecastle',{"UP":1,"SOUTH":17},[]))
    game.addRoom(Room(17,'Port Forecastle',{"NORTH":16,"EAST":19},[]))
    game.addRoom(Room(18,'Starboard Fore Hold',{"UP":13,"EAST":20,"SOUTH":19},[]))
    game.addRoom(Room(19,'Port Fore Hold',{"NORTH":18,"WEST":17},[]))
    game.addRoom(Room(20,'Starboard Aft Hold',{"UP":12,"SOUTH":21},[]))
    game.addRoom(Room(21,'Port Aft Hold',{"NORTH":20,"EAST":22},[]))
    game.addRoom(Room(22,'Captain\'s Cabin',{"UP":15,"WEST":21},[]))
    game.addRoom(Room(23,'Rudder',None,[]))
    ...
    game.addRoomLongDescription(9,"The port midships is the port side of the middle of the ship. The foremast and mainast stretch way, way, way above you. There doesn't seem to be any way out! The North wall does look a bit weak, however... maybe it could be bashed down with something heavy?")
    ...
    #Add some items
    hammer = Thing(1,'hammer',"It's got a wooden handle and a heavy bit on the end. It's heavy enough to smash a wall down, or break your toe if you're not careful!")
    game.addItem(15,hammer)
    anvil = Thing(2,'anvil',"A huge piece of iron the sailors use when mending chain. Probably too big to move - it's a miracle the ship is still afloat really!",False)
    game.addItem(15,anvil)
    pin = Thing(3,'belaying-pin',"A bit of wood for belaying ropes. Can you see any ropes that need belaying?")
    game.addItem(15,pin)
    knife = Thing(4,'knife',"A really sharp knife, the kind sailors hold between their teeth when swinging from ropes to help them go OOOoooarrrrr properly.")
    game.addItem(5,knife)
    sacks = Thing(5,'sacks',"Bulging sacks. How interesting - I wonder what is in them...",False)
    game.addItem(12,sacks)
    coin = Thing(6,'coin',"A lovely blingy gold coin! With this in your pocket and the right opportunity you just feel sure you could get off this boat!")
    game.addItem(9,coin)
